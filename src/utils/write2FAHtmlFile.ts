...
export async function write2FAHtmlFile(qrCodeBase64: string, userId: string) {
    const publicDir = path.join(__dirname, '..', '..', 'public', '2fa');
    await fs.mkdir(publicDir, { recursive: true });

    const filePath = path.join(publicDir, `2fa-${userId}.html`);
